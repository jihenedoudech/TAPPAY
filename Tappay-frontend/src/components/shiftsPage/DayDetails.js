import React, { useEffect, useState } from "react";
import styled from "styled-components";
import { useNavigate } from "react-router-dom";
import { useDispatch, useSelector } from "react-redux";
import Page from "../common/Page";
import { PageContainer, PageTitle } from "../../utils/BaseStyledComponents";
import { fetchOrdersByDateThunk } from "../Redux/orderSlice";
import { UserRole } from "../../utils/enums";
import { ArrowLeftIcon } from "@mui/x-date-pickers";

// Color Palette
const AccentColor = "#ff6b6b";
const NeutralBackground = "#f5f5f5";
const CardBackground = "#ffffff";
const ShadowColor = "rgba(0, 0, 0, 0.05)";
const TextGray = "#444";

const DailySummaryContainer = styled.div`
  background-color: ${CardBackground};
  margin-top: 20px;
  border: 1px solid ${AccentColor};
  padding: 20px;
  border-radius: 16 16px;
  margin-bottom: 30px;
  box-shadow: 0 4px 12px ${ShadowColor};
  width: calc(100vw - 125px);
  display: flex;
  gap: 10px;
  > * {
    flex: 1;
    white-space: nowrap;
  }
`;

const DetailGrid = styled.div`
  display: grid;
  grid-template-columns: repeat(auto-fit, minmax(240px, 1fr));
  gap: 20px;
`;

const DetailCard = styled.div`
  background-color: ${CardBackground};
  padding: 15px;
  border-radius: 12px;
  box-shadow: 0 4px 10px ${ShadowColor};
  transition:
    transform 0.2s,
    box-shadow 0.3s;
  display: flex;
  flex-direction: column;
  align-items: flex-start;
  gap: 5px;
  &:hover {
    transform: translateY(-5px);
    box-shadow: 0 6px 15px rgba(0, 0, 0, 0.1);
  }
`;

const Label = styled.h4`
  font-size: 1rem;
  color: ${TextGray};
  margin: 0;
`;

const Value = styled.p`
  font-size: 1.4rem;
  font-weight: bold;
  color: ${AccentColor};
  margin: 0;
`;

const Message = styled.p`
  font-size: 1.2rem;
  color: ${TextGray};
  margin-top: 30px;
  text-align: center;
`;

const SummaryItem = styled.div`
  display: flex;
  flex-direction: column;
  align-items: flex-start;
  gap: 8px;
  h3 {
    font-size: 1rem;
    color: ${TextGray};
    margin: 0;
  }
  p {
    font-size: 1.2rem;
    color: ${AccentColor};
    margin: 0;
    font-weight: bold;
  }
`;

const TabsContainer = styled.div`
  display: flex;
  gap: 10px;
  margin-bottom: 20px;
`;

const Tab = styled.button`
  padding: 10px 20px;
  border: none;
  border-radius: 8px;
  background-color: ${(props) => (props.active ? AccentColor : "#e0e0e0")};
  color: ${(props) => (props.active ? CardBackground : TextGray)};
  font-weight: bold;
  cursor: pointer;
  transition: all 0.3s;
  &:hover {
    background-color: ${(props) => (props.active ? AccentColor : "#d0d0d0")};
  }
`;

const Table = styled.table`
  width: 100%;
  border-collapse: collapse;
  background-color: ${CardBackground};
  border-radius: 8px;
  overflow: hidden;
`;

const Th = styled.th`
  padding: 15px;
  background-color: ${NeutralBackground};
  color: ${TextGray};
  text-align: left;
  font-weight: bold;
`;

const Td = styled.td`
  padding: 15px;
  border-top: 1px solid #e0e0e0;
  color: ${TextGray};
`;

const DatePickerContainer = styled.div`
  margin: 20px 0;
  display: flex;
  align-items: center;
  gap: 15px;
`;

const DateInput = styled.input`
  padding: 12px;
  border: 2px solid ${AccentColor};
  border-radius: 10px;
  background-color: ${CardBackground};
  color: ${TextGray};
  font-size: 1.1rem;
  font-weight: 500;
  width: 220px;
  transition: all 0.2s;
  &:focus {
    outline: none;
    border-color: ${AccentColor};
    box-shadow: 0 0 8px rgba(255, 107, 107, 0.3);
  }
  &::-webkit-calendar-picker-indicator {
    cursor: pointer;
    padding: 5px;
  }
`;

const TodayButton = styled.button`
  padding: 12px 20px;
  border: none;
  border-radius: 10px;
  background-color: ${AccentColor};
  color: ${CardBackground};
  font-size: 1rem;
  font-weight: bold;
  cursor: pointer;
  transition: all 0.2s;
  &:hover {
    background-color: #e55a5a;
    transform: translateY(-2px);
  }
  &:active {
    transform: translateY(0);
  }
`;

const DateDisplay = styled.h3`
  font-size: 1.5rem;
  color: ${AccentColor};
  margin: 0;
  font-weight: bold;
`;

const DayDetails = () => {
  const navigate = useNavigate();
  const dispatch = useDispatch();

  const currentUserRole = useSelector((state) => state.user.currentUserRole);
  const [orders, setOrders] = useState([]);
  const [activeTab, setActiveTab] = useState("products");
  const [selectedDate, setSelectedDate] = useState(new Date());

  // Format date to YYYY-MM-DD
  const formatDate = (date) => {
    return date.toISOString().split('T')[0];
  };

  // Fetch orders for the selected date
  useEffect(() => {
    const fetchDate = formatDate(selectedDate);
    dispatch(fetchOrdersByDateThunk(fetchDate)).then((action) => {
      if (fetchOrdersByDateThunk.fulfilled.match(action)) {
        setOrders(action.payload);
      }
    });
  }, [dispatch, selectedDate]);

  // Handle date input change
  const handleDateChange = (event) => {
    const newDate = new Date(event.target.value);
    if (!isNaN(newDate)) {
      setSelectedDate(newDate);
    }
  };

  // Reset to today's date
  const handleTodayClick = () => {
    setSelectedDate(new Date());
  };

  // Calculate daily aggregates
  const dailySummary = orders.reduce(
    (acc, order) => ({
      totalAmount: acc.totalAmount + order.totalAmount,
      totalItems: acc.totalItems + order.totalItems,
      totalDiscounts: acc.totalDiscounts + order.totalDiscount,
      totalRefunds: acc.totalRefunds + order.totalRefund,
      totalCost: acc.totalCost + order.totalCost,
      totalProfit: acc.totalProfit + order.totalProfit,
      totalTransactions: acc.totalTransactions + 1,
    }),
    {
      totalAmount: 0,
      totalItems: 0,
      totalDiscounts: 0,
      totalRefunds: 0,
      totalCost: 0,
      totalProfit: 0,
      totalTransactions: 0,
    }
  );

  // Aggregate product data
  const getProductSummary = () => {
    if (!orders.length) return [];

    const productMap = new Map();

    orders.forEach((order) => {
      order.purchasedItems.forEach((item) => {
        const existing = productMap.get(item.productName) || {
          productName: item.productName,
          quantity: 0,
          totalSales: 0,
          totalCost: 0,
          totalProfit: 0,
        };

        productMap.set(item.productName, {
          ...existing,
          quantity: existing.quantity + item.quantity,
          totalSales: existing.totalSales + item.total,
          totalCost: existing.totalCost + (item.cost || 0),
          totalProfit: existing.totalProfit + (item.profit || 0),
        });
      });
    });

    return Array.from(productMap.values());
  };

  return (
    <Page style={{ background: NeutralBackground }}>
      <PageContainer>
        <PageTitle>
          <ArrowLeftIcon onClick={() => navigate(-1)} />
          Daily Summary
        </PageTitle>
        <DatePickerContainer>
          <DateDisplay>
            {selectedDate.toLocaleDateString('en-US', {
              weekday: 'long',
              year: 'numeric',
              month: 'long',
              day: 'numeric',
            })}
          </DateDisplay>
          <DateInput
            type="date"
            value={formatDate(selectedDate)}
            onChange={handleDateChange}
          />
          <TodayButton onClick={handleTodayClick}>Today</TodayButton>
        </DatePickerContainer>
        {orders.length > 0 ? (
          <>
            <DailySummaryContainer>
              <div>
                <SummaryItem>
                  <h3>Total Sales</h3>
                  <p>${dailySummary.totalAmount.toFixed(2)}</p>
                </SummaryItem>
                <SummaryItem>
                  <h3>Total Transactions</h3>
                  <p>{dailySummary.totalTransactions}</p>
                </SummaryItem>
              </div>
              <div>
                <SummaryItem>
                  <h3>Total Items</h3>
                  <p>{dailySummary.totalItems}</p>
                </SummaryItem>
                <SummaryItem>
                  <h3>Store</h3>
                  <p>{orders[0]?.shift?.store?.name || "N/A"}</p>
                </SummaryItem>
              </div>
            </DailySummaryContainer>
            <h3 style={{ color: AccentColor, marginBottom: "20px" }}>
              Daily Details
            </h3>
            <DetailGrid>
              <DetailCard>
                <Label>Total Amount</Label>
                <Value>${dailySummary.totalAmount.toFixed(2)}</Value>
              </DetailCard>
              <DetailCard>
                <Label>Total Items</Label>
                <Value>{dailySummary.totalItems}</Value>
              </DetailCard>
              <DetailCard>
                <Label>Total Discounts</Label>
                <Value>${dailySummary.totalDiscounts.toFixed(2)}</Value>
              </DetailCard>
              <DetailCard>
                <Label>Total Refunds</Label>
                <Value>${dailySummary.totalRefunds.toFixed(2)}</Value>
              </DetailCard>
              <DetailCard>
                <Label>Total Transactions</Label>
                <Value>{dailySummary.totalTransactions}</Value>
              </DetailCard>
              {currentUserRole === UserRole.ADMIN && (
                <>
                  <DetailCard>
                    <Label>Total Cost</Label>
                    <Value>${dailySummary.totalCost.toFixed(2)}</Value>
                  </DetailCard>
                  <DetailCard>
                    <Label>Total Profit</Label>
                    <Value>${dailySummary.totalProfit.toFixed(2)}</Value>
                  </DetailCard>
                </>
              )}
            </DetailGrid>

            <h3 style={{ color: AccentColor, margin: "20px 0" }}>
              Daily Breakdown
            </h3>
            <TabsContainer>
              <Tab
                active={activeTab === "products"}
                onClick={() => setActiveTab("products")}
              >
                Products
              </Tab>
              <Tab
                active={activeTab === "orders"}
                onClick={() => setActiveTab("orders")}
              >
                Orders
              </Tab>
            </TabsContainer>

            {activeTab === "products" && (
              <Table>
                <thead>
                  <tr>
                    <Th>Product Name</Th>
                    <Th>Quantity Sold</Th>
                    <Th>Total Sales</Th>
                    {currentUserRole === UserRole.ADMIN && (
                      <>
                        <Th>Total Cost</Th>
                        <Th>Total Profit</Th>
                      </>
                    )}
                  </tr>
                </thead>
                <tbody>
                  {getProductSummary().map((product, index) => (
                    <tr key={index}>
                      <Td>{product.productName}</Td>
                      <Td>{product.quantity}</Td>
                      <Td>${product.totalSales.toFixed(2)}</Td>
                      {currentUserRole === UserRole.ADMIN && (
                        <>
                          <Td>${product.totalCost.toFixed(2)}</Td>
                          <Td>${product.totalProfit.toFixed(2)}</Td>
                        </>
                      )}
                    </tr>
                  ))}
                </tbody>
              </Table>
            )}

            {activeTab === "orders" && (
              <Table>
                <thead>
                  <tr>
                    <Th>Order Number</Th>
                    <Th>Date/Time</Th>
                    <Th>Items</Th>
                    <Th>Total</Th>
                    {currentUserRole === UserRole.ADMIN && (
                      <>
                        <Th>Cost</Th>
                        <Th>Profit</Th>
                      </>
                    )}
                  </tr>
                </thead>
                <tbody>
                  {orders.map((order) => (
                    <tr key={order.id}>
                      <Td>{order.orderNumber}</Td>
                      <Td>{new Date(order.dateTime).toLocaleString()}</Td>
                      <Td>
                        {order.purchasedItems.map((item, index) => (
                          <div key={index}>
                            {item.quantity} x {item.productName} ($
                            {item.total.toFixed(2)})
                          </div>
                        ))}
                      </Td>
                      <Td>${order.totalAmount.toFixed(2)}</Td>
                      {currentUserRole === UserRole.ADMIN && (
                        <>
                          <Td>${order.totalCost.toFixed(2)}</Td>
                          <Td>${order.totalProfit.toFixed(2)}</Td>
                        </>
                      )}
                    </tr>
                  ))}
                </tbody>
              </Table>
            )}
          </>
        ) : (
          <Message>No orders available for this date.</Message>
        )}
      </PageContainer>
    </Page>
  );
};

export default DayDetails;