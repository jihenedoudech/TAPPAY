...
import { addUserThunk, updateUserThunk } from "../../Redux/userSlice"; // Example redux slice for user actions
...
const UserForm = () => {
  ...
  useEffect(() => {
    ...
    if (location.state && location.state.user) {
      const retrievedUser = location.state.user;
      const assignedStoresIds =
        retrievedUser.assignedStores?.map((store) => store.id) || [];
      setUser({
        ...retrievedUser,
        assignedStoresIds,
      });
  ...
  console.log("user of user form: ", user);
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await dispatch(
      isUpdate
        ? updateUserThunk({ id: user.id, user: userWithoutNulls })
        : addUserThunk(userWithoutNulls)
    );
